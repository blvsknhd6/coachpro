...
export default function RecapTracking({ athleteId, blocId, coachMode = false }) {
  ...
  // Helper : couleur d'une cellule de bilan
  const cc = (value, key) => metricColor(value, key, objectifs, bornes)

  // Formatage : arrondi intelligent
              ...
              <tbody>
                {bilans.map(b => (
                  <tr key={b.name} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className={`px-3 py-3 text-right ${cc(b.sport, 'seances')}`}>{b.sport}j</td>
                    <td className={`px-3 py-3 text-right ${cc(b.kcal, 'kcal')}`}>{fmt(b.kcal, true)}</td>
                    <td className={`px-3 py-3 text-right ${cc(b.proteines, 'proteines')}`}>{fmt(b.proteines)}</td>
                    <td className={`px-3 py-3 text-right ${cc(b.glucides, 'glucides')}`}>{fmt(b.glucides)}</td>
                    <td className={`px-3 py-3 text-right ${cc(b.lipides, 'lipides')}`}>{fmt(b.lipides)}</td>
                    <td className={`px-3 py-3 text-right ${cc(b.sommeil, 'sommeil')}`}>{fmt(b.sommeil)}h</td>
                    <td className={`px-3 py-3 text-right ${cc(b.pas, 'pas')}`}>
                      {b.pas ? Math.round(b.pas).toLocaleString('fr') : '—'}
                    </td>
                    <td className={`px-3 py-3 text-right ${cc(b.stress, 'stress')}`}>{fmt(b.stress)}/10</td>
                  </tr>
                ))}
              </tbody>
            </table>
                  ...
                  <tbody>
                    {[...data].sort((a, b) => new Date(b.date) - new Date(a.date)).map(e => (
                      <tr key={e.id}
                        ...
                        <td className={`px-3 py-2 text-center ${cc(e.kcal, 'kcal')}`}>{e.kcal ?? '—'}</td>
                        <td className={`px-3 py-2 text-center ${cc(e.proteines, 'proteines')}`}>{e.proteines ?? '—'}</td>
                        <td className={`px-3 py-2 text-center ${cc(e.glucides, 'glucides')}`}>{e.glucides ?? '—'}</td>
                        <td className={`px-3 py-2 text-center ${cc(e.lipides, 'lipides')}`}>{e.lipides ?? '—'}</td>
                        <td className={`px-3 py-2 text-center ${cc(e.sommeil, 'sommeil')}`}>{e.sommeil ?? '—'}</td>
                        <td className={`px-3 py-2 text-center ${cc(e.pas_journaliers, 'pas')}`}>{e.pas_journaliers ?? '—'}</td>
                        <td className={`px-3 py-2 text-center ${cc(e.stress, 'stress')}`}>{e.stress ?? '—'}</td>
