...
export default function CoachDashboard() {
  ...
  async function fetchAthletes() {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('*, blocs(id, name, created_at)')
  ...
  async function handleAddAthlete(e) {
    ...
    setErr('')
    try {
      // Inscription via l'API publique (le coach reçoit les identifiants)
  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold">Mes coachées</h1>
        <button
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <h2 className="text-base font-semibold mb-4">Nouvelle coachée</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          <p className="text-sm">Aucune coachée pour l'instant.</p>
        </div>
      ) : (
