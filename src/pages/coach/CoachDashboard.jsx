...
export default function CoachDashboard() {
  ...
  const genreIcon = (genre) => genre === 'femme' ? '♀' : '♂'
                  ...
                  {['homme', 'femme'].map(g => (
                    <button key={g} type="button" onClick={() => setForm(f => ({ ...f, genre: g }))}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${
                        form.genre === g ? 'bg-brand-600 text-white border-brand-600' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                      }`}>
                      {g === 'homme' ? '♂ Homme' : '♀ Femme'}
                    </button>
                  ))}
            ...
            <Link key={a.id} to={`/coach/athlete/${a.id}`}
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-medium ${
                  a.genre === 'femme' ? 'bg-pink-100 text-pink-700' : 'bg-brand-100 text-brand-700'
                }`}>
