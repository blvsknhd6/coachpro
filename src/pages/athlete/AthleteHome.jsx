...
export default function AthleteHome() {
  ...
  async function fetchAll() {
    ...
    const [blocsRes, trackingRes, streakRes] = await Promise.all([
      supabase.from('blocs').select('id, name, objectifs_bloc(*)').eq('athlete_id', profile.id).order('created_at', { ascending: false }).limit(1),
      supabase.from('data_tracking').select('*').eq('athlete_id', profile.id).eq('date', today).single(),
      supabase.from('data_tracking').select('date').eq('athlete_id', profile.id).eq('sport_fait', true).order('date', { ascending: false }).limit(30),
    ])
    ...
    if (blocs?.[0]) {
      ...
      setObjectifs(obj)
      // Load semaines separately (non-blocking for first render)
  ...
  async function fetchSemaines(blocId) {
    const { data: semaines } = await supabase.from('semaines').select('id, numero').eq('bloc_id', blocId).order('numero')
    if (!semaines?.length) { setLoading(false); return }

    // Find active semaine efficiently
    ...
    for (let i = semaines.length - 1; i >= 0; i--) {
      ...
      const { count } = await supabase.from('series_realisees').select('id', { count: 'exact', head: true })
        .eq('athlete_id', profile.id).in('exercice_id', scIds.map(s => s.id))
      if (count > 0) { activeSem = semaines[i]; break }
    }
...
Estimation basée sur des portions standard si non précisé.`,
      ...
      // Get bloc_id for data_tracking sync
      const { data: blocs } = await supabase.from('blocs').select('id').eq('athlete_id', profile.id).order('created_at', { ascending: false }).limit(1)
      ...
      // Sync tracking
      const newTotals = { ...totalMacros, kcal: totalMacros.kcal + (macros.kcal || 0) }
      if (bloc_id) {
        await supabase.from('data_tracking').upsert({
          athlete_id: profile.id, date: today, bloc_id,
  ...
  async function addFavori(repas) {
    const { data: blocs } = await supabase.from('blocs').select('id').eq('athlete_id', profile.id).order('created_at', { ascending: false }).limit(1)
    await supabase.from('repas').insert({ athlete_id: profile.id, date: today, description: repas.description, kcal: repas.kcal, proteines: repas.proteines, glucides: repas.glucides, lipides: repas.lipides })
    await fetchRepasJour()
    ...
    <Layout>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
                ...
                {seances.filter(s => s.nom !== 'Bonus').map(sc => {
                  ...
                  return (
                    <Link key={sc.id} to={`/athlete/seance/${sc.id}/semaine/${activeSemaine?.id}`}
                      ...
                      <span className={`text-xs ${complete ? 'text-green-600 font-medium' : 'text-gray-400'}`}>{complete ? 'Termine' : `${done}/${total}`}</span>
                    </Link>
                  )
                })}
              </div>
          ...
          {/* Widgets custom */}
          {prefs?.home_widgets?.filter(w => w.custom && w.enabled).map(w => (
            <div key={w.id} className="bg-white border border-gray-100 rounded-xl p-4">
              <p className="text-sm font-medium text-gray-700">{w.label}</p>
              <p className="text-xs text-gray-400 mt-1">Widget personnalisé</p>
            </div>
          ))}
        </div>
      )}
