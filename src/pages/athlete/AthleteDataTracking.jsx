...
export default function AthleteDataTracking() {
  ...
  async function fetchBlocs() {
    ...
    setBlocs(data || [])
    if (data && data.length > 0) setActiveBloc(data[0])
  }
  ...
  function field(key, label, unit, type = 'number', opts = {}) {
    const val = form[key]
    const obj = objectifs?.[key]
    const numVal = Number(val)
    const numObj = Number(obj)
    let accent = ''
    if (val !== '' && obj && type === 'number') {
      if (numVal < numObj * 0.9) accent = 'border-amber-300 bg-amber-50'
      else if (numVal > numObj * 1.1) accent = 'border-rose-300 bg-rose-50'
      else accent = 'border-green-300 bg-green-50'
    }
    ...
    return (
      <div>
        ...
        {obj && <p className="text-xs text-gray-400 mt-0.5">Objectif : {obj}{unit ? ' ' + unit : ''}</p>}
      </div>
    )
  }
      ...
      {!activeBloc ? (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            >
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                form.sport_fait ? 'border-brand-500 bg-brand-500' : 'border-gray-300'
              }`}>
                {form.sport_fait && (
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              {field('sommeil', 'Sommeil', 'h', 'number', { step: '0.5', min: '0', max: '24' })}
              {field('pas_journaliers', 'Pas journaliers', '')}
              <div>
              ...
              {field('poids', 'Poids (optionnel)', 'kg', 'number', { step: '0.1' })}
            </div>
          </div>
            ...
            <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {recentEntries.map(e => (
                      <tr key={e.id} className={`border-b border-gray-50 hover:bg-gray-50 cursor-pointer transition-colors ${e.date === selectedDate ? 'bg-brand-50/50' : ''}`}
                        ...
                        <td className="px-3 py-2 text-center text-gray-600">{e.sommeil ?? '—'}h</td>
                        <td className="px-3 py-2 text-center text-gray-600">{e.pas_journaliers ? Number(e.pas_journaliers).toLocaleString('fr') : '—'}</td>
