...
export default function AthleteSeance() {
  ...
  async function fetchAll() {
    ...
    const { data: bloc } = semaine ? await supabase.from('blocs').select('athlete_id, show_charge_indicative, show_rpe').eq('id', semaine.bloc_id).single() : { data: null }
            ...
            return (
              <button key={act.id} onClick={() => toggleActivite(act.id, done)}
                className={`w-full flex items-center gap-3 p-4 rounded-xl border text-left transition-all ${
                  done ? (theme.isFemme ? 'bg-pink-50 border-pink-200 text-pink-800' : 'bg-brand-50 border-brand-200 text-brand-800') : 'bg-white border-gray-100 text-gray-700'
                }`}>
    ...
    <Layout>
      ...
      <div className="mb-4">
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          <span>{doneSeries} / {totalSeries} séries</span><span>{pct}%</span>
        </div>
      ...
      <div className="px-4 py-3 border-b border-gray-50 flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            ...
            {showRpe && exercice.rpe_cible && <span className="ml-1 text-gray-500">· @{exercice.rpe_cible}</span>}
          </p>
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ml-2 flex-shrink-0 ${
          series.length >= exercice.sets ? 'bg-green-50 text-green-700' : series.length > 0 ? 'bg-amber-50 text-amber-700' : 'bg-gray-50 text-gray-400'
        }`}>{series.length}/{exercice.sets}</span>
      </div>
      <div className="px-4 py-3 space-y-2">
        {series.map((s, i) => (
          <SerieRow key={s.id} serie={s} index={i + 1} prevSerie={prevSeries[i] || null}
            repos={exercice.repos} repRange={exercice.rep_range} theme={theme}
  ...
  return (
    <div className="space-y-1 pb-2 border-b border-gray-50 last:border-0 last:pb-0">
      ...
      {prevSerie && (prevSerie.charge || prevSerie.reps) && (
        <div className="pl-7">
          <p className="text-xs text-gray-300">S-1 : {prevSerie.charge ? `${prevSerie.charge}kg` : '—'} × {prevSerie.reps || '—'}{prevSerie.notes ? ` · ${prevSerie.notes}` : ''}</p>
        </div>
      )}
    </div>
  )
    ...
    <div className={`fixed bottom-4 left-4 right-4 rounded-2xl p-4 z-50 flex items-center gap-4 shadow-xl ${isDone ? 'bg-green-600' : remaining <= 10 ? 'bg-red-600' : 'bg-gray-900'} text-white`}>
      <div className="relative w-14 h-14 flex-shrink-0">
        <svg className="w-14 h-14 -rotate-90" viewBox="0 0 56 56">
          ...
          <circle cx="28" cy="28" r="24" fill="none" stroke={isDone ? '#ffffff' : remaining <= 10 ? '#fca5a5' : '#818cf8'} strokeWidth="4"
      ...
      <div className="flex gap-2">
        {!isDone && <button onClick={() => setRunning(r => !r)} className="text-sm bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg">{running ? '⏸' : '▶'}</button>}
