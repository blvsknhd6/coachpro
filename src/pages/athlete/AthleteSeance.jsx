...
export default function AthleteSeance() {
  ...
  const [prevSemaineId, setPrevSemaineId] = useState(null)
  const [prevSeanceId, setPrevSeanceId] = useState(null)
  ...
  const [athletePoids, setAthletePoids] = useState(null)
  ...
  async function fetchAll() {
    ...
    // Feature 3 : récupérer le poids de l'athlète
    ...
    setAthletePoids(
      poidsData?.[0]?.poids ||
      pAthleteProfile?.poids ||
      null
    )

    if (bloc?.powerlifting && semaine?.bloc_id) {
      const { data: maxData } = await supabase
        .from('powerlifting_maxes')
        .select('lift, max_kg')
        .eq('bloc_id', semaine.bloc_id)
        .eq('athlete_id', athId)

      const maxMap = {}
      ;(maxData || []).forEach(m => {
        maxMap[m.lift] = Number(m.max_kg)
      })
    ...
    ;(srData || []).forEach(s => {
      if (map[s.exercice_id]) {
        map[s.exercice_id].push(s)
      }
    })

    setSeries(map)

    await fetchSeriesPrev(
      exs || [],
      semaineId,
      athId,
      sc?.nom,
  ...
  async function fetchSeriesPrev(
    exs,
    currentSemaineId,
    athId,
    seanceNom,
    ...
    ;(srPrevReal || []).forEach(s => {
      ...
      if (ex != null && mapByOrdre[ex.ordre] !== undefined) {
        mapByOrdre[ex.ordre].push(s)
      }
    })
  ...
  async function saveCardio(formValues) {
    ...
    const { error } = await supabase
      ...
      .upsert({
        ...
        activite: formValues.activite || null,
        duree: formValues.duree !== '' ? Number(formValues.duree) : null,
        distance: formValues.distance !== '' ? Number(formValues.distance) : null,
  ...
  async function addSerie(exerciceId, numeroSet) {
    ...
    const poidsCorporsKg =
      (ex?.poids_corps && athletePoids)
        ? Number(athletePoids)
        : null
    ...
    setSeries(prev => ({
      ...
      [exerciceId]: [
        ...(prev[exerciceId] || [])
          .filter(s => s.numero_set !== numeroSet),
        data
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-4 max-w-md">
          ...
          <div>
            ...
            {!cardioSaving && !isComplete && (
              <p className="text-xs text-gray-400">
                Renseigne au moins une information : durée, distance ou note de séance.
              </p>
            )}
          </div>
        </div>
  ...
  // ── Vue Bonus ───────────────────────────────────────────────────────
  if (seance?.nom === 'Bonus') {
    const doneCount = activites.filter(
      a => activitesRealisees[a.id]
    ).length
    ...
    return (
      <Layout>
        ...
        <p className="text-sm text-gray-400 mb-4">
          {doneCount}/{activites.length} réalisées
        </p>
        ...
        <div className="space-y-2">
          {activites.map(act => {
            ...
            return (
              <button
                key={act.id}
                onClick={() => toggleActivite(act.id, done)}
          ...
          {addingActivite ? (
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setAddingActivite(false)}
            ...
            <button
              onClick={() => setAddingActivite(true)}
  ...
  // ── Vue séance normale ──────────────────────────────────────────────
  const totalSeries = exercices.reduce(
    (acc, ex) => acc + ex.sets,
    0
  )

  const doneSeries = Object.values(series).reduce(
    (acc, arr) => acc + arr.filter(s => s.reps || s.charge).length,
    0
  )

  const pct = totalSeries > 0
    ? Math.round((doneSeries / totalSeries) * 100)
    : 0

  const hasSeriesPrev = Object.values(seriesPrev).some(
    arr => arr.some(s => s.charge || s.reps)
  )
    ...
    <Layout>
      ...
      <div className="space-y-3">
        {exercices.map(ex => (
          <ExerciceCard
            ...
            onUpdate={(serieId, field, val) =>
              updateSerie(serieId, ex.id, field, val)
            }
            onDelete={serieId =>
              deleteSerie(serieId, ex.id)
            }
            onOpenCalc={(defaultCharge, defaultReps) =>
              setShowCalc({
                lift: ex.main_lift,
                defaultCharge,
                defaultReps
      ...
      {showCalc && (
        <Calculator1RM
          ...
          currentMax={
            showCalc.lift
              ? powerMaxes[showCalc.lift]
              : null
          }
          onSaveMax={
            showCalc.lift
              ? kg => saveNewMax(showCalc.lift, kg)
              : null
          }
...
function RecapSemainePrev({
  ...
  const exsAvecPerfs = exercices.filter(
    ex => (seriesPrev[ex.ordre] || [])
      .some(s => s.charge || s.reps)
  )
  ...
  function goToPrevSeance() {
    if (prevSeanceId && prevSemaineId) {
      navigate(
        `/athlete/seance/${prevSeanceId}/semaine/${prevSemaineId}`
      )
    }
  }
...
function ExerciceCard({
  ...
  const doneCount = series.filter(
    s => s.reps || s.charge
  ).length

  const isPartial =
    doneCount > 0 &&
    doneCount < exercice.sets

  const isComplete =
    doneCount >= exercice.sets

  const tonnageLabel =
    exercice.poids_corps && athletePoids
      ...
      <div className="px-4 py-3 border-b border-gray-50 flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {exercice.unilateral && (
              <span className="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded font-medium flex-shrink-0">
                ×2
              </span>
            )}
            ...
            {isPowerlifting && exercice.main_lift && (
              <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-medium flex-shrink-0">
                {exercice.main_lift === 'squat'
                  ? '🏋️'
                  : exercice.main_lift === 'bench'
                    ? '💪'
                    : '⚡'}{' '}
                {exercice.main_lift}
              </span>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            ...
            {showChargeIndicative &&
              exercice.charge_indicative && (
                <span className="ml-1 text-gray-500">
                  · {exercice.charge_indicative}kg indic.
                </span>
              )}

            {showRpe && exercice.rpe_cible && (
              <span className="ml-1 text-gray-500">
                · @{exercice.rpe_cible} cible
              </span>
            )}
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              {maxForLift && (
                <span className="text-xs text-amber-600 font-medium">
                  Max réf : {maxForLift}kg
                </span>
              )}

              {best1RM && (
                <span className="text-xs text-green-600 font-medium">
                  ~1RM session : {best1RM}kg
                </span>
              )}
              ...
              <button
                onClick={() =>
                  onOpenCalc(
                    series[series.length - 1]?.charge,
                    series[series.length - 1]?.reps
                  )
                }
          ...
          >
            ...
            {prevSeries[series.length] && (
              <span className="ml-2 text-xs text-gray-300">
                S-1 :{' '}
                {prevSeries[series.length].charge
                  ? `${prevSeries[series.length].charge}kg`
                  : '—'}{' '}
                ×{' '}
                {prevSeries[series.length].reps || '—'}
                {prevSeries[series.length].rpe
                  ? ` @${prevSeries[series.length].rpe}`
                  : ''}
              </span>
            )}
...
function SerieRow({
  ...
  const pctOfMax =
    isPowerlifting && maxForLift && charge
      ? Math.round(
          (Number(charge) / maxForLift) * 100
        )
      : null

  const liveEst1RM =
    isPowerlifting && charge && reps
      ? epley1RM(
          charge,
          reps,
          rpe || null
  ...
  const livePoidsCorpsTonnage =
    isPoidsCorp && athletePoids && reps
      ? Math.round(
          (
            Number(athletePoids) +
            (Number(charge) || 0)
  ...
  return (
    <div className="space-y-1 pb-2 border-b border-gray-50 last:border-0 last:pb-0">
      <div className="flex items-center gap-2">
        <span className="w-5 text-xs font-medium text-gray-400 text-center flex-shrink-0">
          {index}
        </span>
        ...
        <button
          onClick={onDelete}
          className="text-gray-200 hover:text-red-400 text-xl flex-shrink-0"
        >
          ×
        </button>
      </div>
      ...
      <div className="flex gap-2 items-center pl-7">
        <input
          type="text"
          value={notes}
          placeholder="Note (facile, douleur…)"
          onChange={e =>
      ...
      {livePoidsCorpsTonnage && (
        <div className="pl-7">
          <p className="text-xs text-blue-500 font-medium">
            Tonnage : {livePoidsCorpsTonnage}kg
          </p>
        </div>
      )}

      {prevSerie &&
        (prevSerie.charge || prevSerie.reps) && (
          <div className="pl-7">
        ...
        <div className="p-5 space-y-4">
          ...
          {est ? (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
              ...
              {currentMax && (
                ...
                >
                  {pctChange > 0 ? '+' : ''}
                  {pctChange}% vs max actuel ({currentMax}kg)
                </p>
              )}
          ...
          {onSaveMax && est && (
            ...
            >
              {saved
                ? '✓ Max enregistré !'
                : `Enregistrer ${est}kg comme nouveau max`}
            </button>
          )}
        </div>
      </div>
