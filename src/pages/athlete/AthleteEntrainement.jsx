...
export default function AthleteEntrainement() {
  ...
  // Le coach utilise ce même composant pour son propre entraînement :
  // les séances pointent vers /coach/my-training/seance/... au lieu de /athlete/seance/...
  const seanceBasePath = profile?.role === 'coach'
    ? '/coach/my-training/seance'
    : '/athlete/seance'
  ...
  return (
    <Layout>
      ...
      {/* Lien vers l'éditeur — uniquement pour le coach qui regarde son propre programme */}
      {profile?.role === 'coach' && activeBloc && (
        <div className="flex justify-end mb-2">
          <Link to={`/coach/bloc/${activeBloc.id}/edit`}
            className="text-xs text-brand-600 hover:text-brand-800 font-medium border border-brand-200 rounded-lg px-3 py-1.5">
            ✎ Éditer le programme
          </Link>
        </div>
      )}
