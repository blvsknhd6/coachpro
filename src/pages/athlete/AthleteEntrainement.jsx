...
export default function AthleteEntrainement() {
  ...
  async function fetchBlocs() {
    const { data } = await supabase.from('blocs').select('id, name').eq('athlete_id', profile.id).order('created_at', { ascending: false })
    setBlocs(data || [])
  ...
  async function fetchSemaines(blocId) {
    const { data } = await supabase.from('semaines').select('id, numero').eq('bloc_id', blocId).order('numero')
    setSemaines(data || [])
    if (data?.length) {
      let active = data[0]
      for (let i = data.length - 1; i >= 0; i--) {
        ...
        if (count > 0) { active = data[i]; break }
      }
  ...
  async function fetchSeances(semaineId) {
    ...
    const [seancesRes, activitesRes] = await Promise.all([
      supabase.from('seances').select('id, nom, ordre, exercices(id, series_realisees(id)), activites_bonus(id, nom, ordre)').eq('semaine_id', semaineId).order('ordre'),
      supabase.from('activites_realisees').select('activite_id, realisee').eq('athlete_id', profile.id).eq('semaine_id', semaineId),
    ])
  ...
  async function addCustomActivite(seanceId) {
    ...
    const { data } = await supabase.from('activites_bonus').insert({ seance_id: seanceId, nom: newActivite.trim(), ordre }).select().single()
    if (data) {
      setSeances(prev => prev.map(s => s.id === seanceId ? { ...s, activites_bonus: [...(s.activites_bonus || []), data] } : s))
      setNewActivite('')
            ...
            <div className="bg-white border border-gray-100 rounded-xl p-4">
              ...
              {addingActivite ? (
                <div className="flex gap-2">
                  <input autoFocus value={newActivite} onChange={e => setNewActivite(e.target.value)}
                  ...
                  <button onClick={() => setAddingActivite(false)} className="px-3 py-1.5 rounded-lg text-sm border border-gray-200 text-gray-600">Ann.</button>
                </div>
              ) : (
